...
def _get_package_path_and_info(target: str) -> Optional[Tuple[Path, str, Optional[str]]]:
    ...
    try:
        ...
        return Path(pkg_dir), pkg_name, pkg_version
    ...
    try:
        ...
        while root != root.parent:
            if root.name.startswith("sherlock_download_") or root.name.startswith("sherlock_extract_"):
                ...
                logging.debug(f"Cleaned up temp dir: {root}")
                return
            root = root.parent
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if not Path("./config/risk_patterns.yaml").exists():
        Path("./config/risk_patterns.yaml").touch()
    if not Path("./config/approved_packages.yaml").exists():
        Path("./config/approved_packages.yaml").touch()
